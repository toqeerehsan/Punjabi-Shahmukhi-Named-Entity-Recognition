...
history = model.fit([train_sentences_X, train_pos_tags_X], to_categorical(train_tags_y, len(tag2index)), batch_size=128, epochs=10, validation_data=([dev_sentences_X,dev_pos_tags_X], to_categorical(dev_tags_y, len(tag2index))), shuffle = True)
...
scores = model.evaluate([test_sentences_X, test_pos_tags_X], to_categorical(test_tags_y, len(tag2index)))
print(f"{model.metrics_names[1]}: {scores[1] * 100}")

f= open("results/ner_output.conll","w")
predictions = model.predict([test_sentences_X, test_pos_tags_X])
...
os.system("perl results/ner/bin/conlleval < results/ner_output.conll")
