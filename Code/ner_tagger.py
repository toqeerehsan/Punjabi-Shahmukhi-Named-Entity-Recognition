...
model.add(Embedding(input_dim=len(word2index), output_dim=50))
...
#input()
#history = model.fit(train_sentences_X, to_categorical(train_tags_y, len(tag2index)), batch_size=64, epochs=5, validation_data=(dev_sentences_X,to_categorical(dev_tags_y, len(tag2index))), shuffle = True)
history = model.fit(train_sentences_X, to_categorical(train_tags_y, len(tag2index)), batch_size=64, epochs=16, validation_data=(dev_sentences_X,to_categorical(dev_tags_y, len(tag2index))), shuffle = True)
model.save("models/keras_ner_tagger.model")
