...
word_model = gensim.models.KeyedVectors.load_word2vec_format('data/w2v_35m_100d', binary=False)
...
from keras.layers import Dense, LSTM, Dropout, InputLayer, Bidirectional, TimeDistributed, Embedding, Activation, Dropout, concatenate
from keras.layers import Input
from keras.optimizers import Adam,RMSprop

# Defining model with BiLSTM
input_1 = Input(shape=(MAX_LENGTH,))    # input_shape=(2??,)
emb_1= Embedding(input_dim=len(word2index), output_dim=50, input_length=MAX_LENGTH)(input_1)
...
history = model.fit([train_sentences_X, train_chars_X], to_categorical(train_tags_y, len(tag2index)), batch_size=128, epochs=16, validation_data=([dev_sentences_X,dev_chars_X], to_categorical(dev_tags_y, len(tag2index))), shuffle = True)
model.save("models/ner_tagger_charemb.model")

scores = model.evaluate([test_sentences_X,test_chars_X], to_categorical(test_tags_y, len(tag2index)))
...
out_file = open("models/output_no-emb.history",'w')
